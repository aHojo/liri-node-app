...
let displayTweets = function() {
    ...
    client.get("statuses/user_timeline", {screen_name: "KidLiri", count: "20"}, function(err, tweets, response) {
        if (err) {
            return console.log(err);
        }
        for (let i = 0; i < tweets.length; i++) {
            process.stdout.write(`${tweets[i].user.name} Tweeted: ${tweets[i].created_at} on ${tweets[i].text}\n`);
        }
